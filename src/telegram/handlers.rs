...
use crate::{
    ...
    moderation::engine::{analyze_message, ViolationType},
...
async fn handle_language_command(
    ...
    .await {
        ...
        bot.send_message(
            chat_id,
            messages::lang_no_permission(lang),
        )
        .await?;
...
async fn handle_stats_command(
    ...
    let labels =
        messages::stats_labels(lang);


    let stats = match sqlite::get_chat_stats(
        &state.db,
        chat_id.0,
    )
    .await
    {
        ...
        Err(error) => {
            ...
            bot.send_message(
                chat_id,
                messages::violation_generic(lang),
            )
            .await
            .ok();
...
pub async fn message_handler(
    ...
    let event =
        TelegramEvent::from_message(&msg);
    ...
    if let Some(violation) = analyze_message(
        content,
        &event,
        &state,
    )
...
async fn handle_violation(
        ...
        .await
    {
        ...
        Ok(_) => {
            ...
            log::info!(
                "Usuário {} banido por conteúdo proibido",
                user.id
            );
        }
